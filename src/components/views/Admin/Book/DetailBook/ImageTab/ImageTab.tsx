...
const ImageTab = (props: IImageTypes) => {
    const { currentImage, onUpdate, isPendingUpdateBook, isSuccessUpdateBook } = props
    const {
        preview,
    ...
    return (
        <Card className="w-full lg:w-1/2 p-4">
            <CardHeader className="flex flex-col lg:items-start gap-1">
                <h3 className="text-xl font-bold">Book Image</h3>
                <p className="w-full text-small text-default-400">Manage image of this Book</p>
            </CardHeader>
            <CardBody>
                <form className="flex flex-col gap-4" onSubmit={handleSubmitUpdateImage(onUpdate)}>
                    <div className="flex flex-col gap-2">
                        ...
                        <Skeleton isLoaded={!!currentImage} className="aspect-video rounded-lg">
                            <Image src={currentImage} alt="Image for Book" fill className="relative!" />
                        </Skeleton>
                    </div>
                    ...
                    <Button color="primary" type="submit" className="disabled:bg-default-500 mt-2" disabled={isPendingMutateUploadFile || isPendingUpdateBook || !preview}>
                        {isPendingUpdateBook ? (
